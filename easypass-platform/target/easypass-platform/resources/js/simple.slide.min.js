...
(function($) {
	$.fn.simpleSlide = function(options) {
		...
		var _defaults = {
			...
			"windowAction": "fadeIn",
			"imageAction": "fadeIn",
			"loadingImage": domain+"resources/"+"images/1.gif"
		};
		...
		function _clickFunc(target) {
			$(document.body).addClass("none");
			var _IMG_INDEX = _THIS.index(target);
		...
		return {
			add: function( src ) {
				_ALL_IMG_URL.push(src);
				//_THIS.parent().append('<a href="javascript:;" i=' + src + ' class="Slide One"><img src=' + src + ' alt=""></a>');
				_THIS = $(".Slide");
				$("#LookPicture_" + _i).append('<img alt="No (i) attribute or URL error" class="Look_img animated ' + _ATTR.imageAction + '" src="' + src + '" iSrc="' + src + '"/>')
				$(".twoDiv:last").find(".Slide").on("click", function() {
					_clickFunc($(this));
				});
			},
			removeImg : function( $slide ){
				var _index = $slide.index();
				_ALL_IMG_URL.splice(_index,1);
				$slide.remove();
				_THIS = $(".Slide");
				$("#LookPicture_" + _i).find("img").eq( _index ).remove();
			}
		}
